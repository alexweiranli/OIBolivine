...
def cal(fo0,Tcrys0,kd=0.3,P00=1,c_sio2=48,c_alkali=5,c_h2o=0):
    ...
    Tcrys_fo = pd.DataFrame({'Fo': fo0, 'Tcrys': Tcrys0, 'MgO_liq': mg_liq, 'FeO_liq': fe_liq, 'DMgO': np.exp(lnDMg0)},index=[0])
    T_prim = Tcrys0
    ...
    for kk in range(nn):
        ...
        for tt in np.arange(T_prim-10,T_prim+10,0.1):
            lnDMg = -2.158 + 55.09*(P00/tt) - 6.213 * 0.01*c_h2o + 4430/tt + 5.115*0.01*c_alkali
            lnDFe = -3.3 + 47.57*(P00/tt) - 5.192 * 0.01*c_h2o + 3344/tt + 5.595*0.01*c_alkali + 1.633*0.01*c_sio2
            if abs(fe_liq_new*np.exp(lnDFe) + mg_liq_new*np.exp(lnDMg) - 0.667) <= 1e-4:
                T_prim = tt 
                break
        lnDMg = -2.158 + 55.09*(P00/T_prim) - 6.213 * 0.01*c_h2o + 4430/T_prim + 5.115*0.01*c_alkali
        lnDFe = -3.3 + 47.57*(P00/T_prim) - 5.192 * 0.01*c_h2o + 3344/T_prim + 5.595*0.01*c_alkali + 1.633*0.01*c_sio2
        ...
        df__ = pd.DataFrame({'Fo': equilfo, 'Tcrys': T_prim, 'MgO_liq': mg_liq_new, 'FeO_liq': fe_liq_new, 'DMgO': np.exp(lnDMg),'DFeO': np.exp(lnDFe)},index=[0])
